...
module Results
  module Lotos
    RSpec.describe NumberSummary do
      describe 'create' do
        ...
        context '結果が存在する場合' do
          let(:kind) { 'loto6' }
          let(:expected_file_path) { Rails.root.join('tmp/cache/files/test/lotos/loto6/number_summary.json') }
          let(:expected_contents) { file_fixture('lotos/6/number_summary.json').read }
        ...
        context '結果が存在しない場合' do
          let(:kind) { 'mini_loto' }
          let(:expected_file_path) { Rails.root.join('tmp/cache/files/test/lotos/mini_loto/number_summary.json') }
          let(:expected_contents) { file_fixture('lotos/5/empty.json').read }
