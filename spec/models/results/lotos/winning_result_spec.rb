...
module Results
  module Lotos
    RSpec.describe WinningResult do
      describe 'create' do
        before do
          loto1
          loto1_number1
        ...
        context '結果が存在しない場合' do
          let(:kind) { 'mini_loto' }
          let(:expected_file_path) { Rails.root.join('tmp/cache/files/test/lotos/mini_loto/winning_result.json') }
          let(:expected_contents) { file_fixture('lotos/mini_loto/winning_result_empty.json').read }
